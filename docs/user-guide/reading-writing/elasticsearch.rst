...
Once the source processor is on the workflow canvas, drag and drops the "SaveElasticSearch" processor in the workflow. Configure your Elastic Search processor in the dialog box shown below.
...
After configuring the "SaveElasticSearch" processor, connect your data source processor to the Elastic Search processor.
...
Reading data from Elastic Search is easy. Drag and drop the 'ReadElasticSearch' process into your workflow and configure it.
...
In the dialog above, the 'Refresh Schema' button infers the schema of the index. Thus it is able to pass down the output schema to the next processor making it easy to build workflows.

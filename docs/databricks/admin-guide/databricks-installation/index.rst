...
Sparkflows jar has to be uploaded to Databricks. Sparkflows jobs running on Databricks make use of this jar file.
...
Configure the path of the uploaded fire core jar library in Databricks in Sparkflows.
...
Configure app.postMessageURL in Sparkflows
^^^^^^^^^^^^^^^^^^^^

Configure ``app.postMessageURL`` to be the IP of the machine on which Sparkflows is installed. Jobs running on Databricks would post back results to Sparkflows using this URL.
...
Create your token in Databricks. It would be used in making REST API calls to Databricks from Sparkflows.
...
Create Databricks Connection in Sparkflows
^^^^^^^^^^^^^^

Analytical Apps with Databricks Notebook
=========================

The Analytics App in Sparkflows empowers users to initiate the execution of a Notebook in Databricks. The output generated by the Notebook is seamlessly displayed within the Analytical Apps interface. 

During the execution, Sparkflows passes two crucial parameters to the Notebook :

#. **Postback-URL:** A URL that facilitates communication and data exchange between Sparkflows and the Notebook.
...
To leverage the Sparkflows API for data transmission, you need to add the wheel file to your Databricks Notebook. 

This step enables smooth integration and data flow between Sparkflows and Databricks.
...
First create a RestWorkflowContext for communicating with Sparkflows server. Use the following code. ::
...
Sparkflows provides various output functions to present data effectively within the notebook. The following are some examples :

**Output as Text**
........

To output text to Sparkflows, use the following code. ::
...
To display the contents of a PySpark DataFrame as a table in Sparkflows, use the following code. ::
...
To showcase the contents of a Pandas DataFrame as a table in Sparkflows, use the following code. ::
...
Output the chart in Sparkflows by selecting x & y column. It can display Column, Bar & Line chart types as outputs. 
...
To display HTML code in Sparkflows, use the following code. ::
    
    htmlstr1 = "<h3>You can view HTML code in notebooks.</h3>"
 
    restworkflowcontext.outHTML(9, title="Example HTML", text = htmlstr1)
       
**Plotly with Sparkflows**
..........


To output Plotly visualizations to Sparkflows, use the following code. ::
...
The integration between Sparkflows and Databricks enhances collaboration and improves data analysis capabilities.


**DataBricks NoteBook Code Example**
++++++++

The following code example demonstrates the usage of Sparkflows in a Databricks Notebook.
::

...
-----------
Sparkflows MLOps allows you to deploy Machine learning models in Kubeflow. This is also the preferred approach recommended by us. These models can be either trained in Sparkflows or imported from an external system into Sparkflows and served.

Once the configurations are done by Admin, one can deploy the models and the entire pipeline to Kubeflow via few clicks. The steps for the same are below:

#. Import the model into Sparkflows if its an external model or train the model via workflow.
#. Once, the model is trained/imported, they get resgitered to the Sparkflows model registry.

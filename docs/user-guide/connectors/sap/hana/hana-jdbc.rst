...
Sparkflows enables you to read from and write to SAP HANA through HANA JDBC Driver.

Below are the steps to download and configure HANA JDBC Driver and to read and write to HANA in Sparkflows.
...
Configure HANA JDBC Driver in Sparkflows
-----------

Now, the JDBC Driver jar can be configured in Sparkflows to read and write to SAP HANA with the use of JDBC processors.

Below are the steps to configure the driver in Sparkflows in order to read and write to HANA:
...
- Login to the Sparkflows application and go inside the ``Project and Workflow tab`` to create a sample workflow for reading and writing to HANA through Read JDBC and Save JDBC Processors. Select the connection in Read and Save nodes.

...
- a **training workflow** that prepares data, trains and scores the H2O model, and saves it as a MOJO (auto-registering it in MLflow); and
...
#. **Preprocess (recommended)** — add the feature-engineering nodes your data needs.
#. **Split the data** — use the ``Split`` node to divide the data into training and test sets (for example, 80% train / 20% test).
#. **Train the model** — add an H2O algorithm node (such as ``H2O Distributed Random Forest``, ``H2O Gradient Boosting Machine``, or ``H2O Generalized Linear Models``) and connect the training split to it.
#. **Score and evaluate the test data** — add the ``H2O Score`` node and connect the trained model and the test split to it. The ``H2O Score`` node both scores the data and produces the evaluation metrics (confusion matrix, ROC curve, and more). On this node, keep **Use Champion Model** set to ``false``, turn on **Enable Prediction Over Time Metrics**, and set the **Model Category** (Classification, Regression, or Clustering) so the Predictions Over Time graph is populated.
#. **Capture data metrics** — add the ``ML Data Metrics`` node to track data drift, and connect the ``H2O Score`` output to it.
#. **Save the model** — add the ``H2O ML Model Save`` node to persist the trained model as a MOJO.

.. important::

   In the **training** workflow, keep **Use Champion Model** set to ``false`` on the ``H2O Score`` and ``ML Data Metrics`` nodes. You are training and comparing candidate models here — you promote one to champion *afterwards*, in *Select the H2O Champion Model* below.
...
.. figure:: ../../_assets/mlops/end-to-end-examples/h2oml/h2o-training-wf.png
   ...
   The complete H2O training workflow — read the data, split, train, score and evaluate with H2O Score, capture metrics, and save the model.
...
#. **Load the champion** — add the ``H2O ML Model Load`` node.
...
.. important::
   ...
   In the **prediction** workflow, set **Use Champion Model** to ``true`` on the ``H2O ML Model Load``, ``H2O Score``, and ``ML Data Metrics`` nodes so scoring always uses the currently promoted champion.

.. figure:: ../../_assets/mlops/end-to-end-examples/h2oml/associate-scoring-wf.png
   :alt: H2O prediction workflow on the canvas
   :width: 85%

   The prediction workflow — reuse the same preprocessing, load the champion, score the unseen data with H2O Score, and capture drift metrics.

.. figure:: ../../_assets/mlops/end-to-end-examples/h2oml/associate-scoring-wf-1.png
   :alt: H2O prediction node configuration
   :width: 85%

   *Use Champion Model* is set to true on the H2O ML Model Load, H2O Score, and ML Data Metrics nodes, with Prediction Over Time enabled and the Model Category selected on H2O Score.
...
- **Predictions Over Time** — model performance as it scores new data over time (enabled via *Prediction Over Time Metrics* on the ``H2O Score`` node).
